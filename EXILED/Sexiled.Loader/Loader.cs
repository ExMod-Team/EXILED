...
namespace Sexiled.Loader
{
    ...
    public class Loader
    {
        ...
        public Loader()
        {
            ...
            Log.SendRaw($"Sexiled.API - Version {PluginAPI.Loader.AssemblyLoader.Dependencies.FirstOrDefault(x => x.GetName().Name == "Sexiled.API").GetCustomAttribute<AssemblyInformationalVersionAttribute>().InformationalVersion}", ConsoleColor.DarkRed);
        ...
        {
            File.Delete(Path.Combine(Paths.Plugins, "Sexiled.Updater.dll"));
            File.Delete(Path.Combine(Paths.Dependencies, "Sexiled.API.dll"));
        ...
        public static IPlugin<IConfig> CreatePlugin(Assembly assembly)
        {
            try
            {
                foreach (Type type in assembly.GetTypes())
                {
                    ...
                    if (CheckPluginRequiredSexiledVersion(plugin))
                        continue;
        ...
        public static void EnablePlugins()
        {
            ...
            foreach (IPlugin<IConfig> plugin in toLoad.ToList())
            {
                try
                {
                    if (plugin.Name.StartsWith("Sexiled") && plugin.Config.IsEnabled)
                    {
                        plugin.OnEnabled();
        ...
        /// </summary>
        /// <param name="dependencies">The dependencies that could have been loaded by Sexiled.Bootstrap.</param>
        ...
        public IEnumerator<float> Run(Assembly[] dependencies = null)
        {
            ...
            if (!LoaderPlugin.Config.ShouldLoadOutdatedSexiled &&
                ...
                GameCore.Version.BackwardRevision))
            {
                string messageText = new Version(
                    ...
                    AutoUpdateFiles.RequiredSCPSLVersion.Revision)
                            ? "SCP: SL is outdated. Update SCP: SL Dedicated Server to required version or downgrade Sexiled."
                            : "Sexiled is outdated, a new version will be installed automatically as soon as it's available.";

                ServerConsole.AddLog($"{messageText}\nSCP:SL version: {GameCore.Version.VersionString} Sexiled Supported Version: {AutoUpdateFiles.RequiredSCPSLVersion}", ConsoleColor.DarkRed);
                yield break;
            }
            ...
            BuildInfoCommand.ModDescription = string.Join(
                ...
                AppDomain.CurrentDomain.GetAssemblies()
                    .Where(a => a.FullName.StartsWith("Sexiled.", StringComparison.OrdinalIgnoreCase))
        ...
        private static bool CheckPluginRequiredSexiledVersion(IPlugin<IConfig> plugin)
        {
            if (plugin.IgnoreRequiredVersionCheck)
                return false;

            Version requiredVersion = plugin.RequiredSexiledVersion;
            ...
            if (requiredVersion.Major != actualVersion.Major)
            {
                ...
                {
                    Log.Error(
                        $"You're running an older version of Sexiled ({Version.ToString(3)})! {plugin.Name} won't be loaded! " +
                        $"Required version to load it: {plugin.RequiredSexiledVersion.ToString(3)}");
