...
namespace Sexiled.API.Features
{
    ...
    public static class Paths
    {
        ...
        /// <summary>
        /// Gets or sets sexiled directory path.
        /// </summary>
        public static string Sexiled { get; set; }
        ...
        {
            rootDirectory ??= Path.Combine(AppData, "SEXILED");

            Sexiled = rootDirectory;
            Plugins = Path.Combine(Sexiled, "Plugins");
            Dependencies = Path.Combine(Plugins, "dependencies");
            Configs = Path.Combine(Sexiled, "Configs");
            IndividualConfigs = Path.Combine(Configs, "Plugins");
            LoaderConfig = PluginAPI.Loader.AssemblyLoader.InstalledPlugins.FirstOrDefault(x => x.PluginName == "Sexiled Loader")?.MainConfigPath;
            ...
            Log = Path.Combine(Sexiled, $"{Server.Port}-RemoteAdminLog.txt");
        }
