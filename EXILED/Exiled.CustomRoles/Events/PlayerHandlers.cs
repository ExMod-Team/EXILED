...
namespace Exiled.CustomRoles.Events
{
    ...
    public class PlayerHandlers
    {
        ...
        internal void OnSpawned(SpawnedEventArgs ev)
        {
            ...
            foreach (CustomRole role in CustomRole.Registered)
            {
                if (role.Role == ev.Player.Role.Type && !role.IgnoreSpawnSystem && role.SpawnChance > 0 && !role.Check(ev.Player) && (role.SpawnProperties is null || role.SpawnedPlayers < role.SpawnProperties.Limit) && (role.MinPlayers is 0 || Server.PlayerCount + Npc.Count >= role.MinPlayers))
                {
