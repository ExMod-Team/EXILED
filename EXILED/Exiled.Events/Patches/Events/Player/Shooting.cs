...
namespace Exiled.Events.Patches.Events.Player
{
    ...
    internal static class Shooting
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            var isAllowedLabel = generator.DefineLabel();
            var returnLabel = generator.DefineLabel();

            var ev = generator.DeclareLocal(typeof(ShootingEventArgs));

            var offset = -2;
            var index = newInstructions.FindIndex(instruction => instruction.Calls(Method(typeof(IActionModule), nameof(IActionModule.ServerAuthorizeShot)))) + offset;
