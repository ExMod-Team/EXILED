...
namespace Sexiled.Events.Patches.Events.Map
{
    ...
    using Sexiled.Events.Patches.Generic;
    using Footprinting;
    using HarmonyLib;
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            newInstructions.InsertRange(
                ...
                new[]
                {
                    ...
                    // ignore the foreach since sexiled overwrite it
        ...
        private static void ProcessEvent(FlashbangGrenade instance, float distance)
        {
            ...
            foreach (Player player in ReferenceHub.AllHubs.Select(Player.Get))
            {
                if ((instance.transform.position - player.Position).sqrMagnitude >= distance)
                    continue;
                if (!SexiledEvents.Instance.Config.CanFlashbangsAffectThrower && instance.PreviousOwner.CompareLife(player.ReferenceHub))
                    continue;
