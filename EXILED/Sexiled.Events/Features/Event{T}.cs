...
namespace Sexiled.Events.Features
{
    ...
    /// <summary>
    /// An implementation of the <see cref="ISexiledEvent"/> interface that encapsulates an event with arguments.
    /// </summary>
    /// <typeparam name="T">The specified <see cref="EventArgs"/> that the event will use.</typeparam>
    public class Event<T> : ISexiledEvent
    {
        ...
        {
            Log.Assert(Events.Instance is not null, $"{nameof(Events.Instance)} is null, please ensure you have sexiled_events enabled!");
