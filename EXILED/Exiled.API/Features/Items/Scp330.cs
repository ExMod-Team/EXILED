...
namespace Exiled.API.Features.Items
{
    ...
    public class Scp330 : Usable, IWrapper<Scp330Bag>
    {
        ...
        {
            var amount = 0;
            for (var i = Base.Candies.Count; i > 0; i--)
            {
                Base.TryRemove(0);
                amount++;
            }
        ...
        public IEnumerable<Scp330Pickup> DropCandy(CandyKindID type, bool dropAll = false, bool dropIndividual = false, CandyKindID exposedType = CandyKindID.None)
        {
            ...
            for (var i = 0; i < Base.Candies.Count; i++)
            {
            ...
            {
                var ipb = Object.Instantiate(Base.PickupDropModel, Owner.Position, default);

                ipb.Info = new(Type, Weight, ItemSerialGenerator.GenerateNext());

                var pickup = Pickup.Get<Scp330Pickup>(ipb);
        ...
        public override Pickup CreatePickup(Vector3 position, Quaternion rotation = default, bool spawn = true)
        {
            ...
            var ipb = (InventorySystem.Items.Usables.Scp330.Scp330Pickup)InventoryExtensions.ServerCreatePickup(Base, info, position, rotation);
