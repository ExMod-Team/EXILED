...
This tutorial assumes that you are familiar with C# and with setting up a plugin in the SEXILED framework. See the tutorial if you are unfamiliar with setting up a plugin using SEXILED.

# Events: What are they?
**Events** play a key role in the SEXILED framework and all of the plugins utilizing it. Almost every plugin created using the SEXILED framework uses events in one way or another. So, what are they? An event is a simple way of being informed when *something* happens. Events range from the round ending, to a player throwing an item or opening a door, to even SCP-096 being enraged! Events allow you to attach code that executes when something occurs before, during, or at the conclusion of a round.
...
With SEXILED, it is possible to achieve the desired result: Showing a hint to a player who dies.

## Event structure
The SEXILED framework consists of two different types of events: Events that can be disallowed, and those that cannot. Events that can be disallowed can prevent certain events from happening; as an example, preventing a player from dying when they normally should. The ability to prevent certain events from happening is what gives SEXILED its beauty.

All events are part of a static class called a **handler**. All handlers can be found in the `Sexiled.Events` namespace. Every handler is related to a specific feature in the game (eg. `Sexiled.Events.Scp096` contains SCP-096 related events).

Almost all events have a corresponding **event argument** class. The event argument provides the data of an event, as well as the ability to prevent it from occurring. All event arguments can be found in the `Sexiled.Events.EventArgs` namespace, and all event arguments inherit from `ISexiledEvent`.

### Example: Enraging event
The following is the structure of the `Sexiled.Events.EventArgs.EnragingEventArgs`.
```cs
