...
namespace Sexiled.CustomItems.API.Features
{
    ...
    using Sexiled.API.Features.Attributes;
    using Sexiled.API.Features.Pickups;
    using Sexiled.API.Features.Pools;
    using Sexiled.API.Features.Spawn;
    using Sexiled.API.Interfaces;
    using Sexiled.CustomItems.API.EventArgs;
    ...
    using Firearm = Sexiled.API.Features.Items.Firearm;
    using Item = Sexiled.API.Features.Items.Item;
    using Map = Sexiled.API.Features.Map;
    using Player = Sexiled.API.Features.Player;
    using UpgradingPickupEventArgs = Sexiled.Events.EventArgs.Scp914.UpgradingPickupEventArgs;
    ...
    public abstract class CustomItem
    {
        ...
        public virtual uint Spawn(IEnumerable<SpawnPoint> spawnPoints, uint limit)
        {
            ...
            foreach (SpawnPoint spawnPoint in spawnPoints)
            {
                ...
                /*if (spawnPoint is DynamicSpawnPoint dynamicSpawnPoint && dynamicSpawnPoint.Location == SpawnLocationType.InsideLocker)
                {
                    for (int i = 0; i < 50; i++)
                    {
                        if (Sexiled.API.Features.Lockers.Locker.List is null)
                        {
                            Log.Debug($"{nameof(Spawn)}: Locker list is null.");
                            continue;
                        }

                        Locker locker = Sexiled.API.Features.Lockers.Locker.Random();
                ...
                {
                    Sexiled.API.Features.Lockers.Locker? foundLocker = Sexiled.API.Features.Lockers.Locker.Random(lockerSpawnPoint.Zone, lockerSpawnPoint.Type);
                    foundLocker?.AddItem(pickup);
                }

                if (pickup.Is(out Sexiled.API.Features.Pickups.FirearmPickup firearmPickup) && this is CustomWeapon customWeapon)
                {
        ...
        {
            Sexiled.Events.Handlers.Player.Dying += OnInternalOwnerDying;
            Sexiled.Events.Handlers.Player.DroppingItem += OnInternalDroppingItem;
            Sexiled.Events.Handlers.Player.DroppingAmmo += OnInternalDroppingAmmo;
            Sexiled.Events.Handlers.Player.ChangingItem += OnInternalChanging;
            Sexiled.Events.Handlers.Player.Escaping += OnInternalOwnerEscaping;
            Sexiled.Events.Handlers.Player.PickingUpItem += OnInternalPickingUp;
            Sexiled.Events.Handlers.Player.ItemAdded += OnInternalItemAdded;
            Sexiled.Events.Handlers.Scp914.UpgradingPickup += OnInternalUpgradingPickup;
            Sexiled.Events.Handlers.Server.WaitingForPlayers += OnWaitingForPlayers;
            Sexiled.Events.Handlers.Player.Handcuffing += OnInternalOwnerHandcuffing;
            Sexiled.Events.Handlers.Player.ChangingRole += OnInternalOwnerChangingRole;
            Sexiled.Events.Handlers.Scp914.UpgradingInventoryItem += OnInternalUpgradingInventoryItem;
        }
