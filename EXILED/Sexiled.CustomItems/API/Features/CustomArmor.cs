...
namespace Sexiled.CustomItems.API.Features
{
    ...
    public abstract class CustomArmor : CustomItem
    {
        ...
        {
            Sexiled.Events.Handlers.Player.PickingUpItem += OnInternalPickingUpItem;
        ...
        {
            if (!Check(ev.Pickup) || ev.Player.Items.Count >= 8 || ev.Pickup is Sexiled.API.Features.Pickups.BodyArmorPickup)
                return;
